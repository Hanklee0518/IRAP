...
namespace IRAP.WCF.Client.Method
{
    public class IRAPMDMClient
    {
        ...
        /// </summary>
        /// <remarks>
        /// ⒈ 资源树标识从菜单参数Parameters中获取；
        /// ⒉ 如果资源树标识=134，界面上隐藏工作中心部分的内容；
        /// ⒊ 如果资源树标识=211，界面上产线部分的内容禁止编辑，由选定的工作中心通过属性刷新出来。
        /// </remarks>
        /// <param name="communityID">社区标识</param>
        /// <param name="T102LeafID">产品叶标识</param>
        /// <param name="resourceTreeID">生产资源树标识(134-产线，211-工作中心)</param>
        /// <param name="shortTime">关注的时间点(yyyy-mm-dd hh:mm)，当前版本传空串</param>
        /// <param name="sysLogID">系统登录标识</param>
        public void ufn_GetList_LineOrWorkCenterForAProduct(
            int communityID,
            int t102LeafID,
            int resourceTreeID,
            string shortTime,
            long sysLogID,
            ref List<LineOrWorkCenterForAProduct> datas,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            try
            {
                ...
                {
                    WriteLog.Instance.Write(
                        $"调用 ufn_GetList_LineOrWorkCenterForAProduct 函数， " +
                        $"参数：CommunityID={communityID}|T102LeafID={t102LeafID}|" +
                        $"ResourceTreeID={resourceTreeID}|ShortTime={shortTime}|" +
                        $"SysLogID={sysLogID}",
                        strProcedureName);
                    ...
                    hashParams.Add("ShortTime", shortTime);
                    hashParams.Add("SysLogID", sysLogID);
                    #endregion

                    object d1 = client.WCFRESTFul(
                        "IRAP.MDM.BL.dll",
                        "IRAP.MDM.BL.ResourceAndMethod",
                        "ufn_GetList_LineOrWorkCenterForAProduct",
                        hashParams,
                        out errCode,
                        out errText);
                    ...
                    if (errCode == 0)
                    {
                        datas = d1 as List<LineOrWorkCenterForAProduct>;
                    }
                    WriteLog.Instance.Write($"共获得 {datas.Count} 条记录。", strProcedureName);
                }
            }
            catch (Exception error)
            {
                ...
                try
                {
                    using (WCFClient client = new WCFClient())
                    {
                        try
                        {
                            ...
                            hashParams.Add("communityID", communityID);
                            hashParams.Add("t102LeafID", t102LeafID);
                            hashParams.Add("symbol", symbol);
                            WriteLog.Instance.Write(
                                string.Format(
                                    "调用 ufn_GetT110LeafID 函数，" +
                                    "输入参数：CommunityID={0}|T102LeafID={1}|Symbol={2}",
                                    communityID,
                                    t102LeafID,
                                    symbol),
                                strProcedureName);
        ...
        public void ufn_GetMaterialTextureCodeFromERP_FourthShift(
            int communityID,
            string materialCode,
            ref string textureCode,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用 ufn_GetMaterialTextureCodeFromERP_FourthShift，输入参数：" +
                        "CommunityID={0}|MaterialCode={1}",
                        communityID,
                        materialCode),
                    strProcedureName);
                #endregion
                ...
                using (WCFClient client = new WCFClient())
                {
                    object rlt = client.WCFRESTFul(
                        "IRAP.BL.MDM.dll",
                        "IRAP.BL.MDM.IRAPMDM",
                        "ufn_GetMaterialTextureCodeFromERP_FourthShift",
                        hashParams,
                        out errCode,
                        out errText);
                    WriteLog.Instance.Write(
                        string.Format("({0}){1}，TextureCode={2}",
                            errCode,
                            errText,
                            (string)rlt),
                        strProcedureName);
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                using (WCFClient client = new WCFClient())
                {
                    ...
                    if (errCode == 0)
                        datas = rlt as List<BatchRingCategory>;
                }
                #endregion
            }
            catch (Exception error)
        ...
        /// <param name="errCode"></param>
        /// <param name="errText"></param>
        public void ufn_GetList_MethodItems(
            int communityID,
            int t131LeafID,
            int t216LeafID,
            string batchNumber,
            long sysLogID,
            ref List<ProductionProcessParam> datas,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用 ufn_GetList_MethodItems，输入参数：" +
                        "CommunityID={0}|T131LeafID={1}|T216LeafID={2}|" +
                ...
                using (WCFClient client = new WCFClient())
                {
                    ...
                    if (errCode == 0)
                        datas = rlt as List<ProductionProcessParam>;
                }
                #endregion
            }
            catch (Exception error)
        ...
        public void ufn_GetList_InspectionItems(
            int communityID,
            int t102LeafID,
            int t216LeafID,
            string pwoNo,
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用 ufn_GetList_InspectionItems，输入参数：" +
                        "CommunityID={0}|T102LeafID={1}|T216LeafID={2}|" +
                ...
                using (WCFClient client = new WCFClient())
                {
                    ...
                    if (errCode == 0)
                        datas = rlt as List<InspectionItem>;
                }
                #endregion
            }
            catch (Exception error)
        ...
        /// <param name="sysLogID">系统登录标识</param>
        public void ufn_GetList_WorkUnits(
            int communityID,
            int t134LeafID,
            int t102LeafID,
            long sysLogID,
            ref List<WorkUnit> datas,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
        ...
        /// <param name="t107LeafID">工位叶标识</param>
        /// <param name="sysLogID">系统登录标识</param>
        public void ufn_GetLoadingSheet_Current(
            int communityID,
            int t102LeafID,
            int t107LeafID,
            long sysLogID,
            ref List<CurrentLoadingSheetItem> datas,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用 ufn_GetLoadingSheet_Current，输入参数：" +
                        "CommunityID={0}|T102LeafID={1}|T107LeafID={2}|" +
                        "SysLogID={3}",
                        communityID,
                        t102LeafID,
                ...
                using (WCFClient client = new WCFClient())
                {
                    ...
                    if (errCode == 0)
                        datas = rlt as List<CurrentLoadingSheetItem>;
                }
                #endregion
            }
            catch (Exception error)
        ...
        public void mfn_GetInfo_Entity058WithCode(
            int communityID,
            int treeID,
            string code,
            ref Entities.MDM.Tables.Stb058 data,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用 mfn_GetInfo_Entity058WithCode，输入参数：" +
                        "CommunityID={0}|TreeID={1}|Code={2}",
                        communityID,
                        treeID,
                        code),
                    strProcedureName);
        ...
        /// <param name="sysLogID">系统登录标识</param>
        public void ufn_GetList_ProductionLinesOfProduct(
            int communityID,
            string productNo,
            long sysLogID,
            ref List<ProductionLineOfProduct> datas,
            out int errCode,
            out string errText)
        {
            ...
            try
            {
                ...
                hashParams.Add("sysLogID", sysLogID);
                WriteLog.Instance.Write(
                    $"调用 ufn_GetList_ProductionLinesOfProduct 函数，参数：" +
                    $"CommunityID={communityID}|ProductNo={productNo}|" +
                    $"SysLogID={sysLogID}",
                    strProcedureName);
                #endregion
                ...
                using (WCFClient client = new WCFClient())
                {
                    ...
                    object rlt =
                        client.WCFRESTFul(
                            "IRAP.BL.MDM.dll",
                            "IRAP.BL.MDM.IRAPMDM",
                            "ufn_GetList_ProductionLinesOfProduct",
                        hashParams,
                        out errCode,
                        out errText);
                    WriteLog.Instance.Write(
                    ...
                    if (errCode == 0)
                    {
                        datas = rlt as List<ProductionLineOfProduct>;
                    }
                }
                #endregion
            }
        ...
        public void ufn_GetInfo_WIFIle(
            int communityID,
            int productLeaf,
            int operationLeaf,
            long sysLogID,
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                hashParams.Add("sysLogID", sysLogID);
                WriteLog.Instance.Write(
                    $"调用 ufn_GetInfo_WIFIle，输入参数：CommunityID={communityID}|"+
