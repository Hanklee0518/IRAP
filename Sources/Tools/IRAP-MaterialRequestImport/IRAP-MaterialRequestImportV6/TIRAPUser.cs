...
namespace IRAP_MaterialRequestImport
{
    public class TIRAPUser : TStationUser
    {
                        ...
                        try
                        {
                            ...
                            if (errCode == 0)
                            {
                                switch (communityID)
                                {
                                    case -1:
                                        isPWDVerified = false;
                                        throw new Exception($"当前站点[{MacAddress}]未注册");
                                    case 0:
                                        isPWDVerified = false;
        ...
        public string UserDefinedGUI
        {
            get { return userDefinedGUI; }
            set { userDefinedGUI = value; }
        }
        #endregion
        ...
        /// 获取当前用户的可用机构列表
        /// </summary>
        public void GetAvailableAgencies()
        {
            string strProcedureName =
                $"{className}.{MethodBase.GetCurrentMethod().Name}";
        ...
        /// 获取当前用户的可用角色列表
        /// </summary>
        public void GetAvailableRoles(int agencyLeafID)
        {
            string strProcedureName = 
               $"{className}.{MethodBase.GetCurrentMethod().Name}";
            ...
            try
            {
                ...
                roles = 
                    TDBHelper.Instance.sfn_UserRolesInAgency(
                        communityID,
                        userCode,
                        agencyLeafID,
                        out errCode,
