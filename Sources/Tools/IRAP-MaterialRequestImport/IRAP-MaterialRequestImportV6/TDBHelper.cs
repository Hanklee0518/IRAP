...
namespace IRAP_MaterialRequestImport
{
    public class TDBHelper
    {
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    ...
                    $"PlainPWD={plainPWD}|VeriCode={veriCode}|StationID={stationID}",
                    strProcedureName);
                #endregion
                ...
                {
                    errCode = 99000;
                    errText = $"调用 IRAP.dbo.sfn_UserPWDVerify 函数发生异常：{error.Message}";
                    WriteLog.Instance.Write(errText, strProcedureName);
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    ...
                    $"参数：CommunityID={communityID}|UserCode={userCode}",
                    strProcedureName);
                #endregion
                ...
                try
                {
                    using (IRAPSQLConnection conn = new IRAPSQLConnection())
                    {
                        string strSQL = "SELECT * " +
                            "FROM IRAP..sfn_UserAgencies(@CommunityID, @UserCode)";
                ...
                {
                    errCode = 99000;
                    errText = $"调用 IRAP..sfn_UserAgencies 函数发生异常：{error.Message}";
                    WriteLog.Instance.Write(errText, strProcedureName);
                    WriteLog.Instance.Write(error.StackTrace, strProcedureName);
                }
                #endregion
        ...
        /// <param name="communityID">社区标识</param>
        /// <param name="userCode">用户代码</param>
        /// <param name="agencyLeaf">机构叶标识</param>
        /// <param name="errCode"></param>
        /// <param name="errText"></param>
        /// <returns>角色列表集合</returns>
        public List<TRoleInfo> sfn_UserRolesInAgency(
            int communityID,
            string userCode,
            int agencyLeaf,
            out int errCode,
            out string errText)
        {
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    ...
                    $"参数：CommunityID={communityID}|UserCode={userCode}|AgencyLeaf={agencyLeaf}",
                    strProcedureName);
                #endregion
                ...
                try
                {
                    using (IRAPSQLConnection conn = new IRAPSQLConnection())
                    {
                        string strSQL = "SELECT * " +
                            ...
                            "@CommunityID, @UserCode, @AgencyLeaf)";
        ...
        /// <param name="menuItemID">点中的菜单项标识</param>
        /// <param name="scenarioIndex">应用情景（返回）</param>
        /// <param name="refreshGUIOptins">是否刷新界面（返回）</param>
        /// <param name="userDefinedGUI">自定义图形用户界面（返回，BS专用）</param>
        /// <param name="errCode"></param>
        /// <param name="errText"></param>
        public void ssp_RunAFunction(
            ...
            out bool refreshGUIOptins,
            out string userDefinedGUI,
            out int errCode,
            out string errText)
        {
            string strProcedureName =
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                {
                    errCode = 99000;
                    errText = string.Format("调用 ssp_GetSysLogID 发生异常：{0}", error.Message);
                    return;
                }
                #endregion
            }
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                WriteLog.Instance.Write(
                    ...
                    $"CommunityID={communityID}|SysLogID={sysLogID}|UserDiary={1}",
                    strProcedureName);
                #endregion
