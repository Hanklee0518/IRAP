...
namespace Softland.Tools.Print
{
    ...
    public class DeviceInfo
    {
        ...
        /// <summary>
        /// 图像输出支持的颜色范围的像素深度。
        /// 有效值为 1、4、8、24 和 32。默认值为 24。
        /// 仅对 TIFF 呈现支持 ColorDepth，对于其他图像输出格式报表服务器将忽略此设置。
        /// 注意：  
        ...
        /// <summary>
        /// 要为报表设置的页高，以英寸为单位。
        /// 您必须包含一个整数或小数值，后跟“in”（例如，11in）。
        /// 此值将覆盖报表的原始设置。
        /// </summary>
        public int PageHeight { get; set; }
        /// <summary>
        /// 要为报表设置的页宽，以英寸为单位。
        /// 您必须包含一个整数或小数值，后跟“in”（例如，8.5in）。
        /// 此值将覆盖报表的原始设置。 
        /// </summary>
        ...
        {
            string result =
                $"<DeviceInfo><StartPage>{StartPage}</StartPage>" +
                $"<EndPage>{EndPage}</EndPage>"+
                "<OutputFormat>{OutputFormat}}</OutputFormat>"+
                "<DpiX>{DpiX}</DpiX><DpiY>{DpiY}</DpiY></DeviceInfo>";
            return result;
        }
    }
    #endregion
    ...
    public class ZebraPrintHelper
    {
        ...
        public static extern bool OpenPrinter(
            ...
            out IntPtr intptrPrinter, 
            IntPtr intptrPrintDocument);
